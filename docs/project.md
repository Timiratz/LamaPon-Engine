...
初期解像度は起動時のサイズです。実行中はウィンドウの縁をドラッグして変更でき、C++スクリプトの`SetWindowSize()`からも変更できます（[実行中のゲーム画面サイズ](scripting.md#実行中のゲーム画面サイズ)）。
...
「オンラインサービス」カテゴリーでは、Windows x64ゲームのDiscordログインと
アカウント別クラウドセーブを設定します。ゲーム開発者が運用するバックエンドのHTTPS URL、
ゲームID、`production`などの環境IDを指定します。Discordのclient secretやtokenは
プロジェクト設定へ保存せず、バックエンドだけで管理してください。
