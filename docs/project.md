...
エディターの「ファイル」→「プロジェクト設定とビルド...」から次を編集できます。
...
DirectX 12 Experimentalは段階実装中です。書き出したゲームで選ぶと、D3D12で起動Scene、Game Module、Simulationを動かし、Cube／Sphere／Cylinder／Plane／Procedural Meshの基本3D描画（PBR Material、Ambient／Directional／Point／Spot Light、Forward+のクラスタライト、DirectX 11と同じインスタンス描画に対応）、アニメーション／スキニングと内蔵・外部のPNG／JPEG／対応2D DDS textureを含むglTF／GLB／FBX Model、CMO／SDKMESH／VBO Model、アニメーションしないglTF／GLB／FBX Modelのインスタンス描画、Model Rendererのワイヤーフレーム表示、Directional／Spot／Point Lightの影、グラデーション／cubemapのSkyと朝昼夜モードの太陽円盤、cubemapの環境光（GGX事前畳み込みのIBL）、リフレクションプローブ（実行中の6面ベイク、ボックス射影、2個のブレンド）、ベイクした間接光（照度ボリューム）、Sceneの霧、3D ParticleSystem、HDR描画とSSAO、SSR、TAA、Volumetric Light、Depth of Field、カメラMotion Blur、Bloom、Screen Space Lens Flare、自動露出、トーンマッピング／カラーグレーディング、Screen Outline、FXAA、Sprite／Text／Imageなどの2D／UI描画を行います。2DとParticleSystemのcustom pixel shaderは`PSMain`、`t0/s0`（ParticleSystemは`t1`の補助textureも）、`b0`のカスタム値、`b1`のLight2DをD3D11と同じ契約で利用できます。ScreenEffectのcustom shaderは`VSMain`／`PSMain`、`b0`のカスタム値・画面サイズ・深度係数、`t0`のScene色、`t1/t2`の補助texture、`t3`の深度、`s0`をD3D11と同じ4つの差し込み地点で利用できます。ComputeEffectのcustom shaderは`CSMain`、`b0`のカスタム値と出力サイズ、`t0/t1`の入力texture、`s0`、`u0`の名前付きRenderTextureをD3D11と同じ契約で利用できます。DDSはRGBA8／BGRA8／BC1／BC3／BC5の2D texture、6面のcube texture、ミップ列に対応し、array／volumeは未対応です。3D Material custom shaderは、Mesh Rendererでは`VSMain`／`PSMain`／`GSMain`とPlane／Cubeの`HSMain`／`DSMain`、glTF／GLB／FBXのModel RendererではDirectXTK SkinnedEffectと同じ内蔵頂点シェーダーと`PSSkinnedMain`、Material上書き中のCMO／SDKMESH／VBOのModel Rendererでは`VSMain`／`PSMain`で、`b0`〜`b3`、`t0`〜`t25`、`s0`／`s1`、描画状態の宣言、keyword、深度パスをD3D11と同じ契約で描きます（CMO／SDKMESH／VBOでは`VSOutline`／`PSOutline`の輪郭と`PSOccluded`の遮蔽表示、glTF／GLB／FBXでは骨パレットを反映した`VSSkinnedOutline`／`PSOutline`の輪郭と`PSSkinnedOccluded`の遮蔽表示も描きます。Mesh Rendererでは`VSInstancedMain`によるインスタンス描画も行います）。
...
エディターの「ファイル」→「プロジェクト設定とビルド...」→「ビルドプロファイル」からWindowsまたはWebを選び、出力先を指定できます。
「設定して書き出す」を押すと、この画面で変更したプロジェクト設定を検証・保存してから書き出し設定が開きます。
