...
「クリック時のScene操作」で移動先Sceneまたは再読み込みを設定したボタンは、Project Settingsの「シーン遷移」で決めた演出（フェード、ワイプ、アイリス、シェーダーの模様など）でSceneを切り替えます。
「シーン遷移の演出」で「このボタン専用の演出を使う」をオンにすると、そのボタンだけ別の演出にでき、「Gameビューでプレビュー」で見た目を確認できます。
遷移の途中は覆われて見えないボタンや表示し終える前の新Sceneのボタンを押せないため、連打しても二重に移動しません。
演出の種類とC++からの使い方は[SceneとPrefab](scenes.md#シーン遷移演出)を参照してください。
