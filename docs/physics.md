...
エディターの「ウィンドウ」→「解析」→「物理デバッガー」を開くと、物理の状態を目で確かめられます。

- Scene Viewへ接触点と法線（赤＝衝突、黄＝トリガー）、速度（水色）、角速度（紫）を描きます
- 「ボディ」タブで、Rigidbodyとコライダーを持つGameObjectの種類・質量・速さ・スリープ状態・接触数を一覧できます
- 「接触」タブで、直前の物理ステップの接触の組・位置・法線・めり込み量を確認できます

パネルを開いている間だけ接触を記録するため、閉じていれば負荷は掛かりません。
スクリプトからは`Scene::SetPhysicsDebugCaptureEnabled(true)`の後、`Scene::PhysicsDebugContacts()`で同じ情報を読めます。
詳しくは[エディターの解析ツール](editor.md#物理デバッガー)を参照してください。
...
- **ぶつかっているはずなのに反応しない** — 物理デバッガーの「接触」タブに
  その組が出ているかを確認します。出ていなければ衝突マトリクスやColliderの
  有効状態、出ていれば法線の向きとめり込み量を見て原因を切り分けます。
