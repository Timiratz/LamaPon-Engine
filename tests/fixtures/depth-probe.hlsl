...
{
    // 深度が無い環境では青で塗って、テスト側がすぐ気付けるように
    // します（黙って0を読むと「近い」と誤読されます）。
    //
    // 青なのは、距離を書くときにBを一切使わないからです。最初は
    // 赤にしていて、遠景（距離が上限で飽和した画素）がちょうど
    // (255,0,0)になり、正常な絵を「深度が無い」と誤検出しました。
