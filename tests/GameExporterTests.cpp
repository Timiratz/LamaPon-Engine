      ...
      "pixel": {
        ...
        "target": "ps_5_0"
            ...
            std::filesystem::copy_file(
                ...
                runtimeDirectory / "xaudio2_9redist.dll",
                std::filesystem::copy_options::overwrite_existing);
            // Earlier export checks deliberately use tiny placeholder CRT
            // files. The final probe launches the copied executable, so those
            // placeholders must be replaced with the modules backing this
            // test process or the Windows loader fails before main().
            for (const wchar_t* const moduleName : {
                    L"vcruntime140.dll",
                    L"msvcp140.dll" })
            {
                std::filesystem::copy_file(
