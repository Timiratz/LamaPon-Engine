...
//
// HSMain（ハル）とDSMain（ドメイン）を**両方**書くと有効になります。
// 片方だけだと無視されて、ただの板のまま描かれます。
//
// 不透明として宣言している点が要点です。以前はテセレーション経路
// だけ「両面・深度書き込みなし・半透明」で固定されていて、地形の
// ような不透明な面が作れませんでした。
...
// 頂点シェーダーからハルシェーダーへ渡す形。**ここで射影しません。**
