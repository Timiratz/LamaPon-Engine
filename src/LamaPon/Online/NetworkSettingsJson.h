...
namespace LamaPon::Detail
{
    inline nlohmann::json NetworkSettingsToJson(const NetworkConfiguration& settings)
    {
        ...
        return { { "backend", settings.backend == NetworkBackend::Lan ? "Lan" : "EOS" },
    ...
    inline NetworkConfiguration NetworkSettingsFromJson(const nlohmann::json& value)
    {
        ...
        if (backend != "Lan" && backend != "EOS") throw std::invalid_argument("Unknown network backend.");
        result.backend = backend == "Lan" ? NetworkBackend::Lan : NetworkBackend::EpicOnlineServices;
