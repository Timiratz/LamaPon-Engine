...
namespace LamaPon
{
    ...
    // API 58ではReflection Probeのベイク結果とprimary／secondary
    // IBL viewをAPI非依存handleへ移し、Backend世代・cube形状を
    // 描画直前に一括検証する共通契約にしたため、
    // ゲーム用DLLの再ビルドが必要です。
    inline constexpr std::uint32_t GameModuleApiVersion = 58;
