...
namespace LamaPon
{
    ...
    // API 61ではポスト処理のScene入力をAPI非依存型へ移し、
    // raw renderer / RenderTarget入口をprivate互換shimへ移行して
    // 公開frame layoutとaccess contractを更新したため、
    // ゲーム用DLLの再ビルドが必要です。
    inline constexpr std::uint32_t GameModuleApiVersion = 61;
