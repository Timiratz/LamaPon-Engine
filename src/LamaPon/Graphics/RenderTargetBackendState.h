...
// RenderTargetの公開レイアウトから具象描画APIの資源を隠すための
...
{
    // Backendに依存しない公開状態だけを保持します。具象stateはこの
    ...
    // DirectX 11専用のnative資源と処理です。公開RenderTargetはこの型を
    // 知らず、D3D11Backendだけが生成とtransactionalな差し替えを行います。
    struct D3D11RenderTargetState final : RenderTargetBackendState
    {
        ...
        Microsoft::WRL::ComPtr<ID3D11Texture2D> m_depthOfFieldTexture;
        Microsoft::WRL::ComPtr<ID3D11RenderTargetView>
            m_depthOfFieldRenderTargetView;
        Microsoft::WRL::ComPtr<ID3D11ShaderResourceView>
            m_depthOfFieldShaderResourceView;
        Microsoft::WRL::ComPtr<ID3D11Texture2D> m_depthOfFieldBlurTexture;
        Microsoft::WRL::ComPtr<ID3D11RenderTargetView>
