...
namespace LamaPon
{
    ...
    //
    // **カメラの動きだけ**が対象です。物体ごとの速度は持っていないので
    ...
    // シーンでは絞ります。暗い洞窟から外へ出たときに目が慣れていく、
    // あの表現です。
    //
    ...
    struct ScreenSpaceReflectionSettings final
    {
        ...
        // 物の厚みの見積もり（ワールド単位）。レイが物の**側面から**
        // 入った歩は、画面空間では「その物の裏かもしれない」と
        // 区別が付きません。どこまでを裏と見なすかがこの値です。
        // 小さすぎると反射が途中で水平に切れ、大きすぎると物の裏の
        // 床にも映り込みます。
        //
        // 既定はメートル単位の人間サイズの物を想定した1.2です
        // （2026-08-06に鏡像を実際に置いて見比べて決めました。
        // 検証シーンでは1.2で鏡像を100%埋め、そこから上げても
        // 埋まる行は増えません。0.4だと70%で、切れ目がはっきり
        // 見えました）。手すりや板のような薄い物ばかりの場面では
        // 下げてください。
    ...
    // ほんのり赤くなる——直接光が1回はね返った先（バウンス光）が
    // 出ます。UV展開が要らず、動くオブジェクトにも効くのが利点で、
