...
- Windows x64へ2〜4人のホスト主導P2P通信を追加。Direct方式で数値IPv4 / IPv6へ接続し、共有キー認証、ECDH-P256 / HKDF-SHA256 / AES-256-GCMによる暗号化に対応。専用ゲームサーバー・アカウント・SDKは不要。別回線ではホストへ到達できる公開接続先とポート転送、またはIPv6が必要。
- 接続情報共有、接続先とキーの別指定、任意のLAN部屋検索を追加。公開部屋一覧・マッチング用の`INetworkRoomDirectory`拡張インターフェースを用意し、参加UIを招待に限定しない構成にした。公開一覧のサービス自体は含まない。
- 任意のWindows UPnPによるIPv4 TCPポート転送を追加。既定OFF、120秒の短期leaseを更新し、自分が作成した設定だけを終了時に解除する。Windowsファイアウォールは自動変更しない。実ルーター・異なる回線の2台での到達性は未検証。
- `NetworkSession`、`NetworkSceneBridge`、`NetworkIdentityComponent`で位置・回転・拡縮・有効状態・ゲーム用データ、登録Prefab生成／削除、所有権付き操作、途中参加の全状態取得、退出とタイムアウトを扱う。参加者側の物理とゲームScriptを抑制し、停止時にSceneを復元する。
- 定期同期と変更時だけの同期を選択可能にし、オブジェクト不要の`SendCommand`と途中参加対応の`SetSessionState`を追加。協力アクションとターン制の盤面例を用意した。FPS予測・巻き戻し、大規模RTS向け同期、ホスト引き継ぎは含まない。
- エディターへ接続方式・同期方式・LAN公開・UPnP設定、再生中の部屋作成／参加／検索／終了と診断表示を追加。接続UIの状態と検索の寿命を専用パネルが所有する。
- 従来の平文LAN方式と任意のEOSバックエンドを維持。EOSのみ製品設定・SDKが必要で、Windows書き出し時にDLLを確認する。SDK 1.19.2.1で初期化・終了試験を実施済み。EOS認証とインターネット通信は未検証。
- 実TCP / UDP通信、暗号の既知ベクトル・改ざん・リプレイ拒否、4人参加、128オブジェクトの途中参加、キー再作成、平文への降格拒否、Scene同期とゲーム例の回帰試験を追加。Game Module APIは78、Session通信プロトコルは2。全参加者の更新とゲーム用DLLの再ビルドが必要。接続を開始しない既存プロジェクトは従来の動作を維持する。
...
- エディターの「ウィンドウ」へ「解析」サブメニューを追加し、ゲームの動作を調べるパネルをまとめた。どのパネルも既定では閉じており、開いている間だけ記録する。
